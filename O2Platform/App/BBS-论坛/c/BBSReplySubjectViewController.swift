...
class BBSReplySubjectViewController: UIViewController {
    ...
    @IBAction func replySubjectAction(_ sender: UIBarButtonItem) {
        ...
        self.htmlEditorController.getHTML { (c, err) in
            if c is NSString {
...
//        entity.content = self.htmlEditorController.getHTML()
        ...
        AF.request(url!, method: .post, parameters: entity.toJSON(), encoding: JSONEncoding.default, headers: nil).responseJSON { (response) in
